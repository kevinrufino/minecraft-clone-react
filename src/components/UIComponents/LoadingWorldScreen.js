...
export const LoadingWorldScreen = ({buildWorkers,chunksmadecounter})=>{
    ...
    useEffect(()=>{
        ...
        if(loadingScreenHtmlRef.current){
            ...
            // console.log(loadingScreenHtmlRef.currnet.children)
        }

    },[timeSoFar])

    function updateDisplay(){
        loadingScreenHtmlRef.current.children[0].textContent = `load done - ${chunksmadecounter.current.loaddone?'true':'false'} -- ${chunksmadecounter.current.track.count}/${chunksmadecounter.current.track.max}`
